...
class TestLogin:
    ...
    def test_login(self, case):
        if '#name#' in case.case_data:
            case.case_data = replace_data(case.case_data, 'username', get_ini_data('users', 'username'))
        if '#pwd#' in case.case_data:
            case.case_data = replace_data(case.case_data, 'password', get_ini_data('users', 'password'))
...
if __name__ == '__main__':
    pytest.main(['-vs', '--tb=line', './Ftest_login04.py'])
